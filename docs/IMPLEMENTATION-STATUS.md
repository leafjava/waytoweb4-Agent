...
### P11 — browser and HTTP interface rehearsal

Status: complete for offline scope.

- Started the FastAPI and Vite processes together on alternate loopback ports,
  verified both health paths over HTTP and executed the two controlled runs
  against the running server rather than an in-process mock client.
- Updated the controlled-run script to use the current prepare, confirm, mint,
  human-approval, start, drawdown and RedLine API sequence.
- Replaced unsupported Vite directory discovery with a stable
  `/runs/latest.json` endpoint generated by the rehearsal script.
- Removed committed historical mock run payloads. Fresh offline output carries
  simulation IDs and null transaction hashes, so the UI cannot present it as
  current public-chain evidence.
- Restored the intended English/Korean locale switch after the teammate merge
  introduced a duplicate `ko` object and an unavailable Chinese locale. Both
  locales now contain the same 207 keys.
- Added a regression test that executes both controlled runs through the
  current API lifecycle and asserts `DD_LIMIT`, revocation and honest offline
  evidence fields.

Validation: combined Python suite 163 passed; chain 4 passed; frontend
