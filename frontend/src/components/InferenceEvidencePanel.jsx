...
export default function InferenceEvidencePanel({ snapshot }) {
  ...
  return (
    <section className="overflow-hidden rounded-2xl bg-slate-900/70 shadow-xl shadow-black/20">
      <div className="flex flex-col gap-4 border-b border-slate-800 p-5 md:flex-row md:items-start md:justify-between">
        <div>
          <h2 className="text-base font-semibold text-white">
            <i className="fa fa-microchip mr-2 text-cyan-300" aria-hidden="true"></i>
            {t('evidence.title')}
          </h2>
          <p className="mt-1 max-w-2xl text-sm leading-6 text-slate-400">{t('evidence.subtitle')}</p>
        </div>
      ...
      <div className="grid lg:grid-cols-[1.4fr_0.6fr]">
        <div className="min-w-0 border-b border-slate-800 p-5 lg:border-b-0 lg:border-r">
          <div className="mb-3 flex flex-wrap items-end justify-between gap-3">
            <h3 className="text-sm font-medium text-slate-200">{t('evidence.flows')}</h3>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[640px] text-left text-xs tabular-nums">
              ...
              <tbody>
                {flows.map((row) => (
                  <tr key={row.flow} className="border-b border-slate-800/70 last:border-0">
                    <td className="py-2.5 pr-4 font-mono text-cyan-200">{row.flow}</td>
                    <td className="px-3 py-2.5 text-right text-slate-200">{number.format(row.tokens_in)}</td>
                    <td className="px-3 py-2.5 text-right text-slate-200">{number.format(row.tokens_out)}</td>
                    <td className="px-3 py-2.5 text-right text-slate-300">{row.latency_s.toFixed(3)} s</td>
                    <td className="px-3 py-2.5 text-right text-slate-300">{row.energy_Wh_est.toFixed(4)} Wh</td>
                    <td className="py-2.5 pl-4 text-right">
                      <span className={row.usage_source === 'api' ? 'text-emerald-300' : 'text-amber-300'}>
                        {row.usage_source}
                      </span>
                    </td>
                  </tr>
              ...
              <tfoot>
                <tr className="border-t border-slate-700 font-medium text-white">
                  ...
                  <td className="py-3 pl-4 text-right text-slate-400">{total.usage_source || 'none'}</td>
                </tr>
              </tfoot>
            </table>
          </div>
        ...
        <div className="p-5">
          <h3 className="text-sm font-medium text-slate-200">{t('evidence.timeline')}</h3>
          {timeline.length ? (
            <ol className="mt-4 space-y-3">
              {timeline.map((event, index) => (
                <li key={`${event.ts}-${event.kind}-${index}`} className="grid grid-cols-[3.75rem_0.75rem_minmax(0,1fr)] items-center gap-2 text-xs">
                  ...
                  <span className="min-w-0 break-words font-mono text-slate-300">{event.kind}</span>
                </li>
              ))}
            </ol>
          ) : (
          ...
          <div className="mt-5 rounded-xl bg-slate-950 p-3 text-xs leading-5 text-slate-400">
            <span className="text-slate-200">{t('evidence.run')}</span>
...
function EvidenceTag({ label, value, tone }) {
  ...
  return (
    <span className={`rounded-lg px-2.5 py-1.5 ${colors}`}>
      <span className="text-slate-400">{label}</span> <strong className="font-mono font-medium">{value}</strong>
    </span>
  )
}
