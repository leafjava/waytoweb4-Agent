...
export default function SecurityStrip() {
  ...
  return (
    <section>
      ...
      <ol className="md:hidden space-y-3 border-l-2 border-blue-200 pl-4">
        {STAGE_KEYS.map((n) => (
          <li key={n} className="relative">
            ...
            <div className="text-sm font-semibold text-[#1d1d1f]">{t(`sec.${n}.title`)}</div>
            <div className="text-xs text-slate-500 mt-0.5 leading-relaxed">{t(`sec.${n}.body`)}</div>
          </li>
        ))}
      </ol>
    </section>
...
function StepNode({ n }) {
  ...
  return (
    <div className="flex flex-col items-center text-center">
      <div className="relative w-16 h-16 rounded-full bg-white border-2 border-sky-500/70 shadow-lg shadow-sky-500/20 flex items-center justify-center">
        <i className={`fa ${STAGE_ICONS[n]} text-xl text-blue-700`}></i>
        <span className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-emerald-500 text-slate-950 font-bold flex items-center justify-center text-[10px] border-2 border-white">
          ✓
        </span>
      </div>
      <div className="mt-3 text-sm font-semibold text-[#1d1d1f]">{t(`sec.${n}.title`)}</div>
      <div className="mt-1 text-xs text-slate-500 leading-relaxed max-w-[180px]">
        {t(`sec.${n}.body`)}
      </div>
    </div>
  )
